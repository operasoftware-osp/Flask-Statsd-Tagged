...
def _add_metric_to_request(metric_type, metric_name, metric_value):
    if not hasattr(g, "statsd_metrics"):
        g.statsd_metrics = {"timing": [], "incr": [], "gauge": []}
...
class FlaskStatsdTagged(object):
    def __init__(
        ...
        port=8125,
        extra_tags={},
        restrict_paths_to=None,
    ):
        self.app = app
        self.statsd_host = host
        self.statsd_port = port
        self.extra_tags = extra_tags
        self.allowed_paths = restrict_paths_to
        if app is not None:
            self.init_app(app)
    ...
    def connect(self):
        return StatsClient(
            host=self.statsd_host, port=self.statsd_port, maxudpsize=1024
        )
    ...
    def after_request(self, resp):
        ...
        if not request.path or (
            self.allowed_paths and request.path not in self.allowed_paths
        ):
            path = "incorrect"
        else:
            path = request.path

        tags.update({"path": path, "status_code": resp.status_code})
        ...
        with self.pipeline() as pipe:
            ...
            pipe.timing(
                add_tags("flask_usertime", **tags),
                1000 * (rusage.ru_utime - ctx.resource_before.ru_utime),
            )
            pipe.timing(
                add_tags("flask_systime", **tags),
                1000 * (rusage.ru_stime - ctx.resource_before.ru_stime),
            )

            if ctx.content_length != 0:
                pipe.incr(
                    add_tags("flask_request_datarate", **tags), ctx.content_length
                )
                pipe.gauge(
                    add_tags("flask_request_datasizegauge", **tags), ctx.content_length
                )
    ...
    def teardown_request(self, exception=None):
        ...
        try:
            ...
            log.exception(
                "Error while tearing down request. "
